...
1. Run with `substreams-sink-kv`:
    ...
    2023-01-12T10:08:31.803-0500 INFO (sink-kv) sink to kv {"dsn": "badger3:///Users/you/badger_data.db", "endpoint": "mainnet.eth.streamingfast.io:443", "manifest_path": "substreams.yaml", "output_module_name": "kv_out", "block_range": ""}
    2023-01-12T10:08:31.803-0500 INFO (sink-kv) starting pprof server {"listen_addr": "localhost:6060"}
    2023-01-12T10:08:31.826-0500 INFO (sink-kv) reading substreams manifest {"manifest_path": "manifest.yaml"}
...
1. You can checkout and run our connect-web-example like this:
   ...
   Then, from a browser pointing to the shown address, enter a key in the text box. The app only knows how to decode `eth.block_meta.v1.BlockMeta` messages, anything else will be shown as hex-encoded bytes.

2. To decode the value of your own data structures, add your `.proto` files under `proto/` and generate Rust bindings like this:
...
3. Modify the code from `src/App.tsx` to decode your custom type, from this:
